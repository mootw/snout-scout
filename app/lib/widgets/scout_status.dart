...
Map<String, dynamic> getScoutStatus(DataProvider provider) {
  final recentEvents = provider.database.patches.where(
    (patch) =>
        patch.time.isAfter(DateTime.now().subtract(Duration(minutes: 15))),
  );
  final eventsMap = Map.fromEntries([
    ...recentEvents.map((e) => MapEntry(e.identity, (e.time, e.path))),
  ]);
  return eventsMap;
}
